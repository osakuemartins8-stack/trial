...
async function openProductModal(productId) {
    ...
    // Track product view if analytics available
...
async function processCheckout() {
    ...
    try {
        ...
        // Create order - match your database schema
        const orderData = {
            user_id: currentUser.id,
            items: cart.map(item => ({
        ...
        // Redirect to payment demo with amount and order ID
...
async function loadAdminProducts() {
    try {
        ...
        const tbody = document.getElementById('admin-products-body');
        if (tbody) {
            tbody.innerHTML = `<tr><td colspan="7" style="text-align: center; padding: 2rem; color: red;">Error loading products: ${error.message}</td></tr>`;
        }
    }
}
...
async function loadAdminOrders() {
    try {
        ...
        const tbody = document.getElementById('admin-orders-body');
        if (tbody) {
            tbody.innerHTML = `<tr><td colspan="6" style="text-align: center; padding: 2rem; color: red;">Error loading orders: ${error.message}</td></tr>`;
        }
    }
}
    ...
    try {
        ...
        alerts.innerHTML = lowStockProducts.length > 0
            ? lowStockProducts.map(p => `
                <div class="alert-item" style="padding: 0.5rem; background: #fff3cd; border-left: 4px solid #ffc107; margin-bottom: 0.5rem;">
        ...
        tbody.innerHTML = `<tr><td colspan="5" style="text-align: center; padding: 2rem; color: red;">Error loading inventory</td></tr>`;
    }
}
...
function editProduct(productId) {
    ...
    // Show URL section by default for existing products
